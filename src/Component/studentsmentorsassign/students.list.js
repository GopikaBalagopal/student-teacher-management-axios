...
export default function StudentsPage() {
  ...
  const getdata = async () => {
    try {
      let listdata = await axios.get("https://64ec4e09f9b2b70f2bfa11de.mockapi.io/students");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://64ec4e09f9b2b70f2bfa11de.mockapi.io/students/${id}`);
      message.success("Student data Deleted");
      getdata();
    } catch (error) {
      console.log(error);
    }
  ...
  const getProducts = async (id) => {
    console.log(id);
    let singledata = await axios.get(`https://64ec4e09f9b2b70f2bfa11de.mockapi.io/students/${id}`)
    setSolution(singledata.data)
    // fetch("https://64ec4e09f9b2b70f2bfa11de.mockapi.io/students/" + id)
    //   .then((data) => data.json())
          ...
          <div class="cardfortable card shadow">
            ...
            <div className="card-body">
              <table class="table" style={{
                border: "2px solid rgb(0, 0, 0)", margin: "5px",
                ...
                <nav className="TablePagination">
                  <ul class="pagination">
                    ...
                    {numberspage.map((value, index) => {
                      return (
                        <li
                          class={`page-item ${currentpage === value ? "active" : ""
                            }`}
                          key={index}
                        >
                          <a
