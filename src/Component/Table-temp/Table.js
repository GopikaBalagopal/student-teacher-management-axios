...
export default function Table() {
  ...
  const noofpages = Math.ceil(filterData.length / recordsperpage);
  ...
  useEffect(() => {
    if (typeof(filter) === 'string' && filter !== '') {
      const data = userData.filter(obj =>
        ...
        obj.username.toLowerCase().includes(filter.toLowerCase()) ||
        obj.email.toLowerCase().includes(filter.toLowerCase()) ||
        obj.phone.toLowerCase().includes(filter.toLowerCase())
      );
      setFilterData(data)
    }
    else {
